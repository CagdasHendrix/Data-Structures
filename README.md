...
How It Works
Input: The program reads a 15x15 grid from a file named maze.txt.

1 represents a wall.
0 represents a path.

Navigation: It starts from a detected opening in the first row and attempts to reach an opening in the last row.
Backtracking: If the algorithm hits a dead end, it pops the last position from the stack and tries an alternative direction.
